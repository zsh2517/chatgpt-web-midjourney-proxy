...
export const turnstileCheck = async (req :Request, res:Response, next:NextFunction) => {
    ...
    try {
        ...
        if (!Authorization)  {
            throw new Error('无权限访问,请刷新重试 | No access rights by Turnstile');
        }
...
export const regCookie = async (req :Request, res:Response, next:NextFunction)=>{
    try {
        const Authorization = req.header('X-Vtoken');
        if (!Authorization) {
            throw new Error('Turnstile token 缺失');
        }
        ...
        // console.log('outcome>> ', outcome );
        if (!outcome.success)   {
            throw new Error('Turnstile 错误,请刷新重试 | No access rights by Turnstile');
        }
